...
- Optional FP8 mixed precision training
...
# Import optimization utilities for better performance
try:
...
# Import FP8 quantization for memory efficiency
try:
    from deepseek.mlx.quantization import (
        FP8MixedPrecisionManager,
        FP8TrainingConfig,
    )
    HAS_FP8 = True
except ImportError:
...
class TinyModelConfig:
    ...
    memory_cleanup_interval: int = 1  # Clear cache every N epochs
...
class DataLoader:
    ...
    def _load_data(self) -> List[str]:
        ...
        if not data_path.is_absolute():
            # Try to resolve relative to current file location (project root)
            project_root = Path(__file__).resolve().parents[3]  # src/deepseek/mlx -> project root
        ...
        if data_path.is_dir():
            ...
            files = [self.data_path] if self.data_path.exists() else []
        
        if not files:
            raise FileNotFoundError(f"No data files found at {self.data_path} (resolved: {data_path})")
...
class TinyMLXTrainer:
    ...
    - Logging
    - Memory optimization with mx.clear_cache()
    - Optional FP8 mixed precision (--force-fp8)
    """
    ...
    def __init__(
        ...
        # Initialize optimization context if available
        self.training_context = None
        if HAS_OPTIMIZATION:
            self.training_context = MLXTrainingContext(
                eval_every_n_steps=1,
                log_memory=True,
            )
        
        # Initialize FP8 if requested and available
        self.fp8_manager = None
        self.use_fp8 = use_fp8
        if use_fp8 and HAS_FP8:
            print("FP8 mixed precision enabled")
            fp8_config = FP8TrainingConfig()
            self.fp8_manager = FP8MixedPrecisionManager(fp8_config)
        elif use_fp8 and not HAS_FP8:
            print("Warning: FP8 requested but not available")
    
    def _clear_memory(self):
        """Clear MLX cache to free memory."""
        # Force evaluation of any pending computations
        mx.eval(mx.zeros(1))
        # Note: MLX automatically manages memory, but explicit eval helps
        # Clear any Python references that might be holding arrays
        import gc
        gc.collect()
    ...
    def train(
        ...
        while self.global_step < self.max_steps:
            ...
            # Memory cleanup at the end of each epoch (recommended approach)
            if epoch % self.memory_cleanup_interval == 0:
                self._clear_memory()
                print(f"  [Memory cleanup performed after epoch {epoch}]")
        ...
        if self.fp8_manager:
            report = self.fp8_manager.memory_savings_report()
            print(f"FP8 memory savings: {report['savings_ratio']*100:.1f}%")
        print("=" * 60)
    ...
    def load_checkpoint(
        ...
        # Load weights (they are saved in flattened format)
        flat_weights = mx.load(str(path / "model.safetensors"))
        
        # Unflatten weights to match model structure
        # Use instance method via a temporary trainer to access _unflatten_params
        temp_trainer = cls.__new__(cls)
        nested_weights = temp_trainer._unflatten_params(flat_weights)
