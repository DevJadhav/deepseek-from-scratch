...
class WaveBackend(Enum):
    ...
    # CPU backends
    CPU = "cpu"                    # Pure Python CPU-bound (for data prep, light tasks)
    CPU_ONLY = "cpu_only"          # CPU-only PyTorch (no GPU/Metal)
    
    # PyTorch backends
    PYTORCH_CUDA = "pytorch_cuda"  # PyTorch + NVIDIA CUDA (GPU training)
    PYTORCH_MPS = "pytorch_mps"    # PyTorch + Apple MPS (Apple Silicon GPU)
    PYTORCH_CPU = "pytorch_cpu"    # PyTorch CPU-only
    
    # Apple backends
    MLX = "mlx"                    # Apple MLX framework (Apple Silicon optimized)
    
    # Rust backends
    RUST = "rust"                  # Rust native (auto-detect CUDA/Metal/CPU)
    RUST_CUDA = "rust_cuda"        # Rust + NVIDIA CUDA
    RUST_METAL = "rust_metal"      # Rust + Apple Metal
    RUST_CPU = "rust_cpu"          # Rust CPU-only
    
    # Cloud backends
    MODAL_GPU = "modal_gpu"        # Modal cloud GPU (H100/A100)
    
    @classmethod
    def from_backend(cls, backend: "Backend") -> "WaveBackend":
        """Convert Backend enum to WaveBackend."""
        mapping = {
            Backend.PYTORCH_CUDA: cls.PYTORCH_CUDA,
            Backend.PYTORCH_MPS: cls.PYTORCH_MPS,
            Backend.PYTORCH_CPU: cls.PYTORCH_CPU,
            Backend.MLX: cls.MLX,
            Backend.RUST: cls.RUST,
            Backend.MODAL_GPU: cls.MODAL_GPU,
        }
        return mapping.get(backend, cls.CPU)
    
    def is_gpu_backend(self) -> bool:
        """Check if this backend requires GPU."""
        return self in {
            WaveBackend.PYTORCH_CUDA,
            WaveBackend.PYTORCH_MPS,
            WaveBackend.MLX,
            WaveBackend.RUST_CUDA,
            WaveBackend.RUST_METAL,
            WaveBackend.MODAL_GPU,
        }
    
    def is_rust_backend(self) -> bool:
        """Check if this is a Rust backend."""
        return self in {
            WaveBackend.RUST,
            WaveBackend.RUST_CUDA,
            WaveBackend.RUST_METAL,
            WaveBackend.RUST_CPU,
        }
    
    def is_pytorch_backend(self) -> bool:
        """Check if this is a PyTorch backend."""
        return self in {
            WaveBackend.PYTORCH_CUDA,
            WaveBackend.PYTORCH_MPS,
            WaveBackend.PYTORCH_CPU,
        }
...
class TimeSlicedConfig:
    ...
    # Backend to use for all waves (if uniform execution desired)
    default_backend: WaveBackend = WaveBackend.PYTORCH_CUDA
    ...
    def _generate_default_waves(self) -> List[WaveConfig]:
        """
        Generate default waves with all stages for each backend.
        
        Each wave can run all 6 pipeline stages:
        - data_prep: Dataset preparation and tokenization
        - pretrain: Pre-training on large corpus
        - sft: Supervised fine-tuning
        - grpo: Group Relative Policy Optimization (RL)
        - distillation: Knowledge distillation
        - export: Model export (GGUF, CoreML, etc.)
        
        By default, alternates between backends to demonstrate
        cross-backend training capability.
        """
        all_stages = ["data_prep", "pretrain", "sft", "grpo", "distillation", "export"]
        
        return [
            # Wave 1: PyTorch+CUDA - Full training pipeline
            WaveConfig(
                wave_id=1,
            ...
            # Wave 2: MLX - Apple Silicon optimized
            WaveConfig(
                wave_id=2,
            ...
            # Wave 3: Rust - Native high-performance
            WaveConfig(
                wave_id=3,
            ...
            # Wave 4: CPU - Fallback/testing
            WaveConfig(
                wave_id=4,
                backend=WaveBackend.CPU,
    ...
    def single_backend(
        ...
        **kwargs,
        ...
        """
        Create a time-sliced config using a single backend for all waves.
        
        Args:
            backend: Backend to use for all waves
            total_steps: Total training steps
            num_waves: Number of waves to split training into
            **kwargs: Additional TimeSlicedConfig parameters
            
        Returns:
            TimeSlicedConfig with all waves using the specified backend
        """
        steps_per_wave = total_steps // num_waves
        all_stages = ["data_prep", "pretrain", "sft", "grpo", "distillation", "export"]
        ...
        for i in range(num_waves):
            waves.append(WaveConfig(
                wave_id=i + 1,
                backend=backend,
                start_step=i * steps_per_wave,
                end_step=(i + 1) * steps_per_wave,
                stages=all_stages,
                checkpoint_from=f"checkpoints/step_{i * steps_per_wave}/" if i > 0 else None,
            ))
