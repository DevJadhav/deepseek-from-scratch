...
class ModalRunner(BaseRunner):
    ...
    def _sync_data_to_modal(self, local_path: str) -> str:
        ...
        if not local_path_resolved.is_absolute():
            local_path_resolved = get_project_root() / local_path
    ...
    def run(
        ...
        # Use retry logic for Modal commands
        max_retries = extra_config.get("max_retries", 2) if extra_config else 2
        result = self._run_with_retry(modal_cmd, project_root, max_retries)
    ...
    def _run_with_retry(
        ...
        """Run Modal command with retry logic for transient failures."""
        import time

        last_result = None
        for attempt in range(max_retries + 1):
            if attempt > 0:
                print(f"[ModalRunner] Retry attempt {attempt}/{max_retries}")
            ...
            retryable = any(x in error_msg for x in [
                ...
                "transient", "temporary", "unavailable",
            ])

            if not retryable:
                break

            print(f"[ModalRunner] Retryable error detected: {result.stderr[:200]}")
