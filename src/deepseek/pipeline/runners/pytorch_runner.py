...
The runner is invoked by pipeline stages (PretrainStage, SFTStage, etc.)::
    ...
    from deepseek.pipeline.config import PipelineConfig, ModelSize
...
class PyTorchRunner(BaseRunner):
    ...
    def _train_loop_per_worker(train_config: Dict[str, Any]):
        ...
        # Use the actual get_device() from deepseek.torch.training.training
        device = get_device()
    ...
    def run(
        ...
        # Read dataset with optimized settings
        ...
        if use_gpu:
            try:
                ...
                if torch.cuda.is_available():
                    ...
                    # Use 80% of GPU memory for training
                    memory_per_worker = int(gpu_memory_gb * 0.8)
            except Exception:
                pass
        ...
        if use_gpu:
            resources_per_worker["GPU"] = self.config.distributed.gpus_per_worker

        # Add memory constraint if calculated
        if memory_per_worker:
            resources_per_worker["memory"] = memory_per_worker * (1024**3)

        # Configure run settings with proper checkpointing
        run_config = RunConfig(
            name=f"{self.stage}_{self.config.run_name}",
            checkpoint_config=CheckpointConfig(
                num_to_keep=self.config.training.keep_last_n_checkpoints or 3,
            ),
        )
        ...
        trainer = TorchTrainer(
            ...
            scaling_config=ScalingConfig(
                ...
                # SPREAD: distribute workers across nodes for better memory
                # PACK: colocate for better communication (default for non-PP)
