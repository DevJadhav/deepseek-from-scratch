...
- ``deepseek.mlx.attention`` - MultiQueryAttention, GroupedQueryAttention, MultiHeadLatentAttention
- ``deepseek.mlx.moe`` - DeepSeekMoE, DeepSeekMoEV3
- ``deepseek.mlx.mtp`` - MTPModel (Multi-Token Prediction)
- ``deepseek.mlx.grpo`` - GRPOTrainer (Group Relative Policy Optimization)
- ``deepseek.mlx.sft`` - SFTTrainer, SFTConfig
- ``deepseek.mlx.distillation`` - compute_distillation_loss
- ``deepseek.mlx.pipeline`` - ScalingLaws, DataMixer, CurriculumScheduler
...
----------------------
Uses absolute imports from deepseek.mlx package. No sys.path manipulation needed.
"""
...
from deepseek.pipeline.runners.base import BaseRunner, RunnerResult, get_project_root
...
class MLXRunner(BaseRunner):
    ...
    from ``deepseek.mlx`` package using absolute imports.
    ...
    def run(
        ...
        This method imports and uses the MLX modules from deepseek.mlx package.
        ...
        """
        # ============================================================
        # IMPORT ACTUAL MLX IMPLEMENTATION MODULES (ABSOLUTE IMPORTS)
        # ============================================================
        try:
            ...
            # Attention mechanisms (absolute imports from deepseek.mlx)
            ...
            # Mixture of Experts
            from deepseek.mlx.moe import DeepSeekMoE, DeepSeekMoEV3Config
            # Multi-Token Prediction
            from deepseek.mlx.mtp import MTPModel
            # Training utilities
            from deepseek.mlx.grpo import GRPOTrainer
            from deepseek.mlx.sft import SFTTrainer, SFTConfig
            from deepseek.mlx.pipeline import ScalingLaws, DataMixer, CurriculumScheduler
            from deepseek.mlx.distillation import compute_distillation_loss, KDLossType
        ...
        if self.stage == "pretrain":
            ...
            # Use distillation loss from deepseek.mlx
            self.logger.info("Computing distillation loss")
            from deepseek.mlx.distillation import DistillationConfig
