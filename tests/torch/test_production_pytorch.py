"""
DeepSeek-V3.2 Production Hardening Tests - PyTorch Backend
...
class TestSpeculativeDecoding:
    ...
    def test_mtp_model_forward(self):
        ...
        from deepseek.torch.model.mtp import MTPModel, MTPConfig
        
        device = get_device()
        
        # MTPModel takes a config object
        vocab_size = 1000
        d_model = 128
        num_layers = 2
        k_predictions = 4
        ...
        config = MTPConfig(
            ...
            prediction_depth=k_predictions,
        )
        mtp = MTPModel(config).to(device)
        
        batch, seq = 2, 32
        input_ids = torch.randint(0, vocab_size, (batch, seq), device=device)
        
        main_logits, future_logits, _ = mtp(input_ids)
