...
/// Hardware capabilities for automatic precision selection
...
pub struct HardwareCapabilities {
    ...
    /// Total memory in GB
    pub memory_gb: f32,
    /// Whether FP8 is supported (H100+, SM 9.0+)
    pub supports_fp8: bool,
    /// Whether BF16 is supported (A100+, SM 8.0+)
...
impl HardwareCapabilities {
    ...
    #[cfg(feature = "cuda")]
    pub fn detect() -> Self {
        // In CUDA mode, query actual device properties
        // This is a simplified version - real implementation would use cuDeviceGetAttribute
        Self {
            device_name: "CUDA Device".to_string(),
            compute_capability: (8, 0), // Assume A100-class by default
            memory_gb: 40.0,
            supports_fp8: false, // Would need SM 9.0+
            supports_bf16: true,  // SM 8.0+
            supports_fp16: true,
            has_cuda: true,
        }
    }
    ...
    pub fn detect() -> Self {
        // CPU/Metal fallback
        Self {
            device_name: "CPU/Metal".to_string(),
    ...
    /// Get optimal dtype for training
    pub fn get_optimal_dtype(&self) -> &'static str {
    ...
    pub fn check_fp8_support(&self) -> (bool, String) {
        if !self.has_cuda {
            return (false, "CUDA not available. FP8 requires H100/H200 GPU.".to_string());
        }
        
        if self.supports_fp8 {
            (true, format!("FP8 supported on {} (SM {}.{})", 
                self.device_name, self.compute_capability.0, self.compute_capability.1))
        } else {
            let fallback = if self.supports_bf16 { "BF16" } else { "FP16" };
            (false, format!(
                "FP8 not supported on {} (SM {}.{}). Requires H100/H200 (SM 9.0+). Falling back to {}.",
                self.device_name, self.compute_capability.0, self.compute_capability.1, fallback
            ))
        }
    }
}
...
pub fn check_fp8_support() -> (bool, String) {
    HardwareCapabilities::detect().check_fp8_support()
}

// ============================================================================
// Auto Precision Training
// ============================================================================

/// Automatic precision selection based on hardware capabilities
pub struct AutoPrecisionTraining {
    ...
    /// Whether to use FP16
    pub use_fp16: bool,
    /// FP8 manager (if using FP8)
    pub fp8_manager: Option<FP8MixedPrecisionManager>,
}

impl AutoPrecisionTraining {
    /// Create new auto precision trainer
    pub fn new(prefer_fp8: bool) -> Self {
        let caps = HardwareCapabilities::detect();
        
        let use_fp8 = prefer_fp8 && caps.supports_fp8;
        let use_bf16 = !use_fp8 && caps.supports_bf16;
        let use_fp16 = !use_fp8 && !use_bf16 && caps.supports_fp16;
        
        let fp8_manager = if use_fp8 {
            Some(FP8MixedPrecisionManager::new(FP8TrainingConfig::default()))
        } else {
            None
        };
        
        // Log precision mode
        let mode = if use_fp8 { "FP8" } 
            else if use_bf16 { "BF16" } 
            else if use_fp16 { "FP16" } 
            else { "FP32" };
        eprintln!("[AutoPrecision] Using {} on {}", mode, caps.device_name);
    ...
    /// Get precision mode string
    pub fn precision_mode(&self) -> &'static str {
        if self.use_fp8 { "FP8" }
        else if self.use_bf16 { "BF16" }
        else if self.use_fp16 { "FP16" }
        else { "FP32" }
    }
    ...
    pub fn get_precision_report(&self) -> PrecisionReport {
        PrecisionReport {
            device: self.caps.device_name.clone(),
            compute_capability: format!("{}.{}", 
                self.caps.compute_capability.0, 
                self.caps.compute_capability.1),
            precision_mode: self.precision_mode().to_string(),
...
/// Precision configuration report
#[derive(Clone, Debug)]
...
mod tests {
    ...
    fn test_hardware_capabilities_detect() {
        ...
        // FP16 should be widely supported
        assert!(caps.supports_fp16);
        
        // Optimal dtype should be valid
        let dtype = caps.get_optimal_dtype();
        assert!(["bf16", "f16", "f32"].contains(&dtype));
    }
    
    #[test]
    fn test_hardware_capabilities_fp8_check() {
        let caps = HardwareCapabilities::detect();
        let (supported, message) = caps.check_fp8_support();
        
        // Message should be informative
        assert!(!message.is_empty());
        
        // If supported, should mention H100 or similar
        if supported {
            assert!(message.contains("supported") || message.contains("SM"));
        } else {
            // Should indicate why not supported (CUDA not available, or not H100)
            assert!(
                message.contains("not supported") || 
                message.contains("Falling back") ||
                message.contains("CUDA not available"),
    ...
    #[test]
    fn test_check_fp8_support_function() {
        let (supported, message) = check_fp8_support();
        
        // Should return consistent results with HardwareCapabilities
        let caps = HardwareCapabilities::detect();
        let (caps_supported, _) = caps.check_fp8_support();
        
        assert_eq!(supported, caps_supported);
        assert!(!message.is_empty());
    }
    
    #[test]
    fn test_auto_precision_training() {
        let apt = AutoPrecisionTraining::new(true);
        
        // Should have valid precision mode
        let mode = apt.precision_mode();
        assert!(["FP8", "BF16", "FP16", "FP32"].contains(&mode));
        
        // If not using FP8, fp8_manager should be None
        if !apt.use_fp8 {
            assert!(apt.fp8_manager.is_none());
        }
    }
    
    #[test]
    fn test_auto_precision_training_report() {
        let apt = AutoPrecisionTraining::new(false);
        
        let report = apt.get_precision_report();
        
        assert!(!report.device.is_empty());
        assert!(!report.precision_mode.is_empty());
    ...
    fn test_precision_report() {
        ...
        assert_eq!(report.device, "Test GPU");
        assert_eq!(report.precision_mode, "BF16");
        assert!(report.supports_bf16);
    }
}
