...
impl MultiHeadLatentAttention {
    ...
    /// Get d_latent for cache initialization
    pub fn d_latent(&self) -> usize {
        self.d_latent
    }

    /// Forward pass with legacy KV cache (stores full K/V).
    /// Deprecated: Use forward_with_latent_cache for memory efficiency.
    ...
    /// * `latent_cache` - Optional mutable reference to LatentKVCache
    /// 
    /// # Returns
    ...
    pub fn forward_with_latent_cache(
        &self, 
        x: &Tensor, 
        latent_cache: Option<&mut crate::model::kv_cache::LatentKVCache>
    ) -> Result<Tensor> {
        let (batch_size, seq_len, _) = x.dims3()?;
        ...
        // 2. Key/Value Path (MLA) - Compute compressed latent
        let c_kv = self.w_dkv.forward(x)?; // (batch, seq_len, d_latent)

        // 3. Update latent cache if provided, get full cached latent
        let c_kv_full = if let Some(cache) = latent_cache {
            cache.update(&c_kv)?  // Only store compressed latent (~14× smaller)
        } else {
            c_kv
        };
...
impl SequenceParallelDeepSeekAttention {
    ...
    /// 
    /// Implements decoupled RoPE correctly by combining position and content
    /// scores BEFORE softmax, as per DeepSeek-V3 paper specification.
    pub fn forward(&self, x: &Tensor) -> Result<Tensor> {
        let (batch_size, local_seq_len, _) = x.dims3()?;
        ...
        // C: Combine content and position scores BEFORE softmax
        // This is the key fix for decoupled RoPE - scores must be combined pre-softmax
        
        // Compute content scores
        let scale_content = 1.0 / (self.d_head as f64).sqrt();
        let content_scores = (q_c.matmul(&k_c.transpose(2, 3)?)? * scale_content)?;
        ...
        // CORRECT: Combine scores BEFORE softmax (not after)
        // The position scores modulate the content attention, not replace it
        let combined_scores = (content_scores + position_scores)?;
        
        // Apply causal mask for local sequence
        let mask: Vec<u8> = (0..local_seq_len)
            .flat_map(|i| (0..local_seq_len).map(move |j| if j <= i { 1 } else { 0 }))
            .collect();
        let mask = Tensor::from_vec(mask, (local_seq_len, local_seq_len), x.device())?;
        let mask = mask.broadcast_as((batch_size, self.num_heads, local_seq_len, local_seq_len))?;
        
        let neg_inf = Tensor::new(f32::NEG_INFINITY, x.device())?.broadcast_as(combined_scores.shape())?;
        let masked_scores = mask.where_cond(&combined_scores, &neg_inf)?;
        
        // Single softmax on combined scores
        let attn_weights = ops::softmax(&masked_scores, 3)?;
        
        // Compute context using content V
...
mod tests {
    ...
    #[test]
    fn test_decoupled_rope_combined_scores() -> Result<()> {
        // Test that DeepSeekAttention correctly combines content and position scores
        ...
        let attn = DeepSeekAttention::new(d_model, num_heads, d_latent, d_rope, vb)?;
    ...
    #[test]
    fn test_deepseek_attention_causal_mask() -> Result<()> {
        // Verify causal masking works correctly with decoupled RoPE
        let device = Device::Cpu;
        let varmap = VarMap::new();
        let vb = VarBuilder::from_varmap(&varmap, DType::F32, &device);
        ...
        // Create input where later positions have distinctive values
        let x = Tensor::randn(0f32, 1f32, (batch_size, seq_len, d_model), &device)?;
        let output = attn.forward(&x, None)?;
    ...
    #[test]
    fn test_mla_latent_cache() -> Result<()> {
        use crate::model::kv_cache::LatentKVCache;
        ...
        let d_model = 64;
        let num_heads = 4;
        let d_latent = 16;  // 4× compression
        let batch_size = 2;
        let max_seq_len = 128;
        
        let mla = MultiHeadLatentAttention::new(d_model, num_heads, d_latent, vb)?;
        
        // Create latent cache
        let mut cache = LatentKVCache::new(batch_size, max_seq_len, d_latent, DType::F32, &device)?;
        
        // Process first chunk
        let x1 = Tensor::randn(0f32, 1f32, (batch_size, 16, d_model), &device)?;
        let out1 = mla.forward_with_latent_cache(&x1, Some(&mut cache))?;
        assert_eq!(out1.dims(), &[batch_size, 16, d_model]);
        assert_eq!(cache.current_seq_len(), 16);
        
        // Process second chunk (simulating generation)
        let x2 = Tensor::randn(0f32, 1f32, (batch_size, 1, d_model), &device)?;
        let out2 = mla.forward_with_latent_cache(&x2, Some(&mut cache))?;
        assert_eq!(out2.dims(), &[batch_size, 1, d_model]);
        assert_eq!(cache.current_seq_len(), 17);
        
        Ok(())
    }
    
    #[test]
    fn test_mla_latent_cache_memory_efficiency() -> Result<()> {
        use crate::model::kv_cache::{KVCache, LatentKVCache};
        
        let device = Device::Cpu;
        
        let d_model = 512;
        let num_heads = 8;
        let d_latent = 64;  // 8× compression factor
        let batch_size = 4;
        let max_seq_len = 2048;
        let head_dim = d_model / num_heads;
        let dtype_bytes = 4;  // f32
        
        // Full KV cache (legacy)
        let _full_cache = KVCache::new(batch_size, max_seq_len, num_heads, head_dim, DType::F32, &device)?;
        
        // Latent cache (memory efficient)
        let latent_cache = LatentKVCache::new(batch_size, max_seq_len, d_latent, DType::F32, &device)?;
        
        // Calculate memory stats
        let (latent_bytes, full_bytes, ratio) = latent_cache.memory_stats(d_model, num_heads, dtype_bytes);
        
        println!("Memory efficiency test:");
        println!("  d_model={}, d_latent={}, ratio={:.2}×", d_model, d_latent, ratio);
        println!("  Latent cache: {:.2} MB", latent_bytes as f64 / 1e6);
        println!("  Full KV cache: {:.2} MB", full_bytes as f64 / 1e6);
        
        // Verify significant memory savings (should be ~8× for this config)
        assert!(ratio > 6.0, "Expected >6× compression, got {:.2}×", ratio);
        
        Ok(())
    }
    
    #[test]
    fn test_mla_with_and_without_cache_parity() -> Result<()> {
        // Verify that with latent cache produces same output as without cache
        // when processing the same sequence in one shot
        let device = Device::Cpu;
        let varmap = VarMap::new();
        let vb = VarBuilder::from_varmap(&varmap, DType::F32, &device);
        ...
        // Without cache
        let out_no_cache = mla.forward_with_latent_cache(&x, None)?;
        
        // With cache (but processing full sequence at once, so should be equivalent)
        let mut cache = crate::model::kv_cache::LatentKVCache::new(
            batch_size, seq_len + 10, d_latent, DType::F32, &device
        )?;
        let out_with_cache = mla.forward_with_latent_cache(&x, Some(&mut cache))?;
        
        // Outputs should be identical
        let diff = (&out_no_cache - &out_with_cache)?.abs()?.sum_all()?.to_scalar::<f32>()?;
        assert!(diff < 1e-5, "Outputs should be nearly identical, diff={}", diff);
